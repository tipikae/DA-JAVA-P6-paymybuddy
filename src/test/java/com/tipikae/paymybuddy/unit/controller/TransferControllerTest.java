...
import com.tipikae.paymybuddy.controllers.TransferController;
...
class TransferControllerTest {
	...
	void getTransferReturnsErrorWhenUserNotFoundException() throws Exception {
		doThrow(UserNotFoundException.class).when(transferService).getTransfer(anyString());
	...
	void getTransferReturnsTransferWhenOk() throws Exception {
		...
		when(transferService.getTransfer(anyString())).thenReturn(transferDTO);
		mockMvc.perform(get("/transfer"))
			.andExpect(status().isOk())
